...
def initialize_models_at_startup():
    ...
    try:
        ...
        test_output = llm_model(
            "[INST] Say hello [/INST]", max_tokens=10, temperature=0.1, stream=False
        )
    ...
    try:
        ...
        try:
            ...
            is_contact_query = any(
                keyword in query.lower() for keyword in contact_keywords
            )
            ...
            print(
                f"PERFLOG: Selected {len(relevant_chunks)} chunks from Qdrant, context length: {len(context)}"
            )
            print(
                f"PERFLOG: Search scores: {[f'{r.score:.3f}' for r in sorted_results[:5]]}"
            )
            print(
                f"PERFLOG: Query type: {'contact' if is_contact_query else 'general'}"
            )
        ...
        try:
            ...
            def generate_stream():
                try:
                    ...
                    print(
                        f"Streaming completed. Total chunks: {chunk_count}, Answer length: {len(answer)}"
                    )
                    ...
                    if not answer.strip():
                        ...
                        if (
                            "choices" in non_stream_output
                            and len(non_stream_output["choices"]) > 0
                        ):

      ...
      >
        <div className="w-full">
          <div className="relative flex items-center justify-between">
            ...
            <div className="flex w-full items-center px-4 lg:px-0 lg:ml-64">
              {/* Hamburger button - mobile only */}
              <button
                onClick={navbarToggleHandler}
                id="navbarToggler"
                aria-label="Mobile Menu"
                className="block lg:hidden ml-auto mr-2 p-2 rounded focus:outline-none"
              >
              ...
              {navbarOpen && (
                <nav className="absolute top-full left-0 w-full bg-[#232a36] shadow-lg z-50 block lg:hidden">
                    ...
                    {menuData.filter((menuItem) => menuItem.hidden !== true).map((menuItem, index) => (
                      <li key={index} className="group relative">
                        <Link
                          href={menuItem.path || '#'}
                          onClick={navbarToggleHandler}
                          className={`block px-6 py-3 text-base text-white group-hover:text-primary dark:text-white dark:group-hover:text-primary ${
                            pathUrl === menuItem?.path && "text-primary"
                          }`}
                        >
                          {menuItem.title}
              ...
              <div className="hidden items-center justify-end pr-12 sm:flex lg:pr-8">
                {/* theme toggler */}
                <button
                ...
                {session?.user ? (
                  <div className="relative" ref={userMenuRef}>
                    <button
                      ...
                      className={`flex items-center space-x-2 px-7 py-3 text-base font-medium ${

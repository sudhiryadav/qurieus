...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const paddleConfig = await prisma.paddleConfig.findUnique({ where: { subscriptionPlanId: plan.id } });
  if (!paddleConfig || !paddleConfig.productId) return NextResponse.json({ error: 'No Paddle productId' }, { status: 400 });
